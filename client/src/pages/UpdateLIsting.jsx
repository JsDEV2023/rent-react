...
export default function CreateListing() {
    ...
    const handleImageSubmit = async (e) => {
        if (files.length > 0 && files.length < 7) {
            ...
            for (let i=0; i < files.length + formData.imageUrls.length; i++) {
            ...
            Promise.all(promises).then(urls => {
                ...
                setImageUplaodError('Image upload failed (2 mb max)')
                setUploading(false)
            })
        } else {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        try {
            if(formData.imageUrls.length < 1) return setError('You must upload at leadt one image')
